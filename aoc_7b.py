...
def hand_type(s: str) -> str:
    ...
    if hand_map.get("J") is not None and hand_map.get("J") != 5:
        find_modified_hand(hand_map)
...
def find_modified_hand(hand_map: dict()) -> None:
    num_joker = hand_map.get("J")
    while num_joker > 0:
        max_frequency = 0
        char = ""
        for key, val in hand_map.items():
            if key != "J" and val > max_frequency:
                max_frequency = val
                char = key
        hand_map[char] += 1
        num_joker -= 1
